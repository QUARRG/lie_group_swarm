...
class Circle(Node):
    def __init__(self):
        ...
        self.va_r = self.r*(2*np.pi/self.Tp)*np.vstack((-np.sin(2*np.pi*self.t/self.Tp+phase),np.cos(2*np.pi*self.t/self.Tp+phase),np.zeros(self.N)))
        self.va_r_dot = -self.r*(2*np.pi/self.Tp)**2*np.vstack((np.cos(2*np.pi*self.t/self.Tp+phase),np.sin(2*np.pi*self.t/self.Tp+phase),np.zeros(self.N)))
    ...
    def takeoff(self,time_th):
        ...
        r[2,:] = self.hover_height*t #+ self.initial_pose.position.z
